...
(define n-block 2)
(define array-size (* n-block 2 warpSize))
...
(define (AOS-sum-fast threadId blockID blockDim I O)
  ...
  (for ([i struct-size])
    (let* ([index (modulo (+ localId i) struct-size)]
           [lane (+ (modulo (+ i (quotient localId 2)) 2) (* localId 2))]
           [x (shfl (get I-cached index) lane)])
...
(define (test)
  ...
  (run-kernel AOS-sum-slow (x-y-z (* 2 warpSize)) (x-y-z n-block) I O*)
  ;(run-kernel AOS-sum-fast (x-y-z (* 2 warpSize)) (x-y-z n-block) I O*)
...
(define (AOS-sum-sketch threadId blockID blockDim I O)
  ...
  (for/bounded ([i (??)])
    (let* ([index (modulo (?index localId (@dup i) 1) struct-size)]  ;(+ localId i)
           [lane (?lane localId (@dup i) [a b c] 4)]  ; (+ (modulo (+ i (quotient localId 2)) 2) (* localId 2))
           [x (shfl (get I-cached index) lane)])
...
(define (synthesis)
  (AOS-sum-spec I O O-sizes)
  (run-kernel AOS-sum-sketch (x-y-z (* 2 warpSize)) (x-y-z n-block) I O*)
...
(define (load-synth)
  ...
  (run-kernel AOS-sum-store (x-y-z (* 2 warpSize)) (x-y-z n-block) IDs)
  ...
  (run-kernel AOS-sum-load (x-y-z (* 2 warpSize)) (x-y-z n-block) I warps)
  (define sol
    (time
     (synthesize
      #:forall (symbolics I)
