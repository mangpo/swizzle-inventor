...
(define (run-with-warp-size spec kernel w n)
  ...
  (run-kernel kernel (x-y-z w 1) (x-y-z 1 n-block) A B C* n)
  ;(pretty-display ">>> C")
  ;(acc-print C)
  ;(pretty-display ">>> C*")
  ;(acc-print C*)
...
(define (mult64-shared-int2 threadId blockID blockDim A B C n)
  (define warpId (get-warpId threadId))
  (define-shared a-cached (create-matrix (* (x-y-z 2 1) blockDim)))
  (define-shared b-cached (create-matrix (* (x-y-z 2 1) blockDim)))
  (define block-offset (* (x-y-z 2 1) blockID blockDim))
...
(define (mult64-shared-sketch2 threadId blockID blockDim A B C n)
  ...
  (for ([i warpSize])
    (let* ([lane-a1 (?fan tidx n
      ...
      (accumulate acc1 (list a1 b1) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc2 (list a1 b1) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc3 (list a1 b1) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc4 (list a1 b1) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc5 (list a1 b1) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc6 (list a1 b1) #:pred (?cond tidx (@dup i) #:mod warpSize))
      
      (accumulate acc1 (list a1 b2) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc2 (list a1 b2) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc3 (list a1 b2) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc4 (list a1 b2) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc5 (list a1 b2) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc6 (list a1 b2) #:pred (?cond tidx (@dup i) #:mod warpSize))
      
      (accumulate acc1 (list a2 b1) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc2 (list a2 b1) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc3 (list a2 b1) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc4 (list a2 b1) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc5 (list a2 b1) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc6 (list a2 b1) #:pred (?cond tidx (@dup i) #:mod warpSize))
      
      (accumulate acc1 (list a2 b2) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc2 (list a2 b2) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc3 (list a2 b2) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc4 (list a2 b2) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc5 (list a2 b2) #:pred (?cond tidx (@dup i) #:mod warpSize))
      (accumulate acc6 (list a2 b2) #:pred (?cond tidx (@dup i) #:mod warpSize))
      ))
